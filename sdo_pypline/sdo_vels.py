...
def calc_velocities_average(con, mag, dop, aia, mask, region_mask=None, v_quiet=None):
    # don't bother doing math if there is nothing in the mask
    if (type(region_mask) is np.ndarray) and (~region_mask.any()):
        return 0.0, 0.0, 0.0, 0.0
    ...
    # TODO add v_mer???
    v_phot = np.nansum(dop.v_rot * (con.image - k_hat_con * con.ldark) * w_active * region_mask) * pix_ratio
    ...
    if v_quiet is None:
        # calculate v_quiet
        v_quiet = np.nansum(dop.v_corr * con.image * w_quiet * region_mask) * pix_ratio
...
def calc_velocities_unweighted(con, mag, dop, aia, mask, region_mask=None, v_quiet=None):
    # don't bother doing math if there is nothing in the mask
    if (type(region_mask) is np.ndarray) and (~region_mask.any()):
        return 0.0, 0.0, 0.0, 0.0
    ...
    # calculate velocity terms
    v_hat = np.nansum(dop.v_corr * region_mask)

    # TODO add v_mer???
    v_phot = np.nansum(dop.v_rot * w_active * region_mask)
    ...
    if v_quiet is None:
        # calculate v_quiet
        v_quiet = np.nansum(dop.v_corr * w_quiet * region_mask)
        v_quiet /= np.nansum(w_quiet * region_mask)
